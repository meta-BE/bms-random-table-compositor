...
ターゲット環境はWindowsだが、開発機の都合上 macOS でも動作する必要がある（コア機能は両OSで利用可能。**タスクバー常駐は Windows / Linux のみ**。macOS は Cocoa/AppKit のメインスレッド要件と Wails が衝突するため、Phase 1 ではトレイ無効化＝ウィンドウクローズで終了する挙動。macOS ネイティブトレイは将来 NSStatusBar を直接統合する形で再検討）。
...
| システムトレイ | `fyne.io/systray` (Windows/Linux のみ。macOS は build tag で no-op スタブ。Phase 1 / Plan 1 で `getlantern/systray` から差し替え - getlantern は macOS で Wails と AppDelegate が衝突するため) |
...
- **シングルインスタンス**: Wails v2 標準の `SingleInstanceLock` オプションを利用（Windows: 名前付き Mutex、macOS/Linux: Unix domain socket）。`UniqueId` を指定するだけでロック・IPC・前面化が統合提供される。`OnSecondInstanceLaunch` で既存ウィンドウを `runtime.WindowShow` + `runtime.Show` で前面化。独自のロックファイル実装は採用しない（Phase 1 / Plan 1 で当初検討した `./.lock` + `syscall.Flock` 方式は撤回）
- **ウィンドウクローズ時 (Windows/Linux)**: Wails の `OnBeforeClose` で `runtime.WindowHide()` を呼び、トレイに格納（要件「タスクバー常駐」を満たす）。HTTPサーバとコアサービスは動き続ける
- **ウィンドウクローズ時 (macOS)**: トレイ無効のため、`OnBeforeClose` は `false` を返してそのままクローズ＝アプリ終了
- **トレイ「終了」メニュー (Windows/Linux)**: `services.Close()` でリソース解放 → `os.Exit(0)` で確実にプロセス終了。`wailsruntime.Quit` はトレイ goroutine から呼ぶと Windows でメインループが正常終了しない症状があったため、強制終了パターンを採用
...
./logs/YYYY-MM-DD.log                 # 日次ローテーションログ
                                      # シングルインスタンスは Wails 標準
                                      # (SingleInstanceLock) で扱うためロック
                                      # ファイルは作成しない
```
...
| シングルインスタンス: 二重起動 | Wails v2 の `SingleInstanceLock` オプションで一元管理。`OnSecondInstanceLaunch` で既存ウィンドウを `runtime.WindowShow` + `runtime.Show` で前面化、新インスタンスは即終了 |
| ウィンドウクローズ (Win/Linux、トレイ稼働中) | OnBeforeClose で `runtime.WindowHide` → トレイのみ稼働。HTTPサーバは継続 |
| ウィンドウクローズ (macOS、トレイ無効) | OnBeforeClose で false 返却 → 通常クローズ＝アプリ終了 |
| トレイ「終了」 | `services.Close()` でDBクローズ・ログクローズ → `os.Exit(0)` で強制終了 |
...
- **PR自動CI（lint/test）**: 今回は導入しない。MVPでは手元の `make test` / `make lint` で運用
- **Windowsクロスビルドワークフロー**: `workflow_dispatch` トリガーで手動発火する `.github/workflows/build-windows.yml` を用意（POC時に作成したワークフローを引き続き活用、ビルド対象を本体に切り替える）。bms-elsaの `build-windows.yml` を参考
- **golangci-lint**: 当面ローカルのみ（CIには載せない）
...
- Windows で動作する exe を **GitHub Actions（`workflow_dispatch`）経由** でビルドし、`gh run download` でArtifactを取得して動作確認
...
**Windowsビルド用ワークフローについて**:

- `.github/workflows/poc-build-windows.yml` を新規作成（bms-elsa の `build-windows.yml` を参考）
- トリガーは `workflow_dispatch` のみ（タグpushや自動CIは不採用、`gh workflow run` で都度手動発火）
- 内容: `windows-latest` runner で `actions/setup-go@v5`（Go 1.24）, `actions/setup-node@v4`（Node 20）, Wails CLI v2.11.0 をインストール → `poc/` ディレクトリで `wails build -platform windows/amd64` → 成果物 exe を `actions/upload-artifact@v4` でArtifactとして保存
- ローカルからは `gh workflow run poc-build-windows.yml` で発火、`gh run watch` で完了待機、`gh run download <run-id>` でArtifact取得

実行例:

```bash
gh workflow run poc-build-windows.yml
gh run watch <run-id>
...
| **PR時の自動CI** | PR時に `go test ./...` / `go vet` / `gofmt -l` を回す。MVPまでは手動運用 |
| **Windowsビルドの自動リリース** | bms-elsa同様、タグpushで自動ビルド＋Releaseアップロード（MVP段階では `workflow_dispatch` 手動発火のみ） |
